...
// session_wire_test.go pins the EXACT reverse-leg invocation contract the runner's
// verb:session seam decodes (plugin-check session_seam.go): class "verb", word
// "session", op OpExecute, and the request JSON in the seam's wire shape. This test
// FAILS if the submission path is deleted or its wire contract drifts — the guard the
// validator asked for (A2/B12: a test that would fail without the reverse-leg
// submission).
