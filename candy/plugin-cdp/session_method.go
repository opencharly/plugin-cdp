...
// session_method.go — the cdp: session method (plan Cutover E, E-3): the
...
// OpExecute — the SAME E3b channel the endpoint resolution
// (cc.ResolveEndpoint) rides, so the provider code is placement-invisible
// (compiled-in vs out-of-process). The recorder is THIS plugin's OWN binary in
// recorder mode (CHARLY_CDP_RECORDER=1; cmd/serve) holding the CDP wire — the
...
// is the PROVIDER-resolved one (the Invoke already ran ResolveEndpoint); the
...
func runSession(ctx context.Context, cc kit.CheckContext, ep *cdpEndpoint, in *params.CdpInput, venueDefault string) (string, error) {
	// session identity: the runner injects session_id for instruments; a PLAN-STEP
	// session (authoring cdp: {method: session, action: start, session_id: x} directly
	// in a plan) falls back to "default" — the same fallback the record-session
	// contract uses (spice's record_name chain has no analogue: cdp has no record
	// method).
	if in.SessionId == "" {
...
// + session identity threaded via the CHARLY_CDP_* env (recorder.go). Extracted
// from sessionStart so the wire request is unit-testable without the reverse leg.
func buildSessionSpawn(in *params.CdpInput, ep *cdpEndpoint, exe, venue, logDir string) sessionRequest {
	fps := in.Fps
	if fps <= 0 {
		fps = 5
	}
...
func sessionStart(ctx context.Context, cc kit.CheckContext, ep *cdpEndpoint, in *params.CdpInput, venueDefault string) (string, error) {
	if in.StateDir == "" {
...
func sessionStop(ctx context.Context, cc kit.CheckContext, in *params.CdpInput) (string, error) {
	if in.StateDir == "" {
	...
	// recorder writes it, which can lag the process exit (a blocked screencast poll
...
// finalize is its own SIGTERM trap — an in-flight screencast event select with
// its ack pacing delays it). 10s is generous for a two-file write and short
...
func sessionStatus(ctx context.Context, cc kit.CheckContext, in *params.CdpInput) (string, error) {
